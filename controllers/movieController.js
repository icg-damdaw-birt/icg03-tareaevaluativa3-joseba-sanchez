...
// POST /api/movies/:id/favorite - Marca/desmarca como favorito
exports.toggleFavorite = async (req, res) => {
  const { id } = req.params;

  try {
    // Primero, obtén la película actual (verificando propiedad)
    const movie = await prisma.movie.findFirst({
      where: { id, ownerId: req.user.userId },
    });
    ...
    // Alterna el estado de isFavorite
...
exports.updateRating = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      error: 'Rating inválido. Debe ser un número entero entre 0 y 5.',
    });
  }
